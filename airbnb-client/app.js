...
var uploadProfileVideo = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb) {
            ...
            uploadMyVideo = "https://s3.amazonaws.com/projectcmpe273/" + file.originalname + "" + Date.now().toString();
            cb(null, file.originalname + "" + Date.now().toString());
        }
    })
});
...
app.post('/editListing', host.editListing);
...
app.post('/adminGetBillfromBillDate', admin.adminGetBillfromBillDate);
app.post('/adminGetBillfromBillMonth', admin.adminGetBillfromBillMonth);
app.post('/adminGetHostfromCity', admin.adminGetHostfromCity);
app.post('/adminGetUserfromCity', admin.adminGetUserfromCity);
...
app.post('/adminGetAllUserDetailsWithRedis', admin.adminGetAllUserDetailsWithRedis);//redis
app.post('/adminGetAllBillDetailsWithRedis', admin.adminGetAllBillDetailsWithRedis);//redis
app.post('/getAuctionableProperties', places);
app.post('/placeBid', trip);
...
app.post('/updateAuctionTrips', trip);
...
app.post('/deleteHost', userProfile);
app.post('/deleteAccount', userProfile);
...
app.post('/uploadProfileVideo', uploadProfileVideo.any(), function (req, res, next) {
    ...
    mq_client.make_request("uploadProfileVideo_queue", {
        "user": req.body.user,
        "videoLink": uploadMyVideo
    }, function (err, result) {
...
app.post('/upload', upload.any(), function (req, res, next) {
    ...
    mq_client.make_request("uploadImage_queue", {
        "listId": req.body.listId.listId,
        "imageLink": imageColl
    }, function (err, result) {
...
app.post('/uploadProfilePic', uploadProfilePic.any(), function (req, res, next) {
    mq_client.make_request("uploadProfileImage_queue", {
        "user": req.body.user,
        "imageLink": imageProfileColl
    }, function (err, result) {
        if (err) {
            console.log(err);
            throw err;
...
app.post('/uploadReview', uploadReview.any(), function (req, res, next) {
    ...
    var msg_payload = {
        ...
        'profilePic': req.body.reviewerPic,
        'listReviewImages': listReviewImages
    };
    req.body.listReviewImages = listReviewImages;
    console.log("In reviewUser  in the APPP", JSON.stringify(req.body));
    mq_client.make_request("reviewList_queue", req.body, function (err, result) {
        if (err) {
            console.log(err);
            throw err;
        } else {
